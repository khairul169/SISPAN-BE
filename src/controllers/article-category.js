...
const getAll = async (req, res) => {
  try {
    ...
    const result = await models.ArticleCategory.findAndCountAll({
      where: filter,
      order: [["name", "ASC"]],
      offset,
      limit,
...
const create = async (req, res) => {
  try {
    const { name } = req.body;
    const result = await models.ArticleCategory.create({ name });
...
const update = async (req, res) => {
  try {
    const item = await models.ArticleCategory.findByPk(req.params.id || 0);
    if (!item) {
